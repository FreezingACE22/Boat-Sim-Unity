...
public class buoyFloater : MonoBehaviour
{
    ...
    [Tooltip("Total number of floaters on the rb.")]
    ...
    void FixedUpdate()
    {
        ...
        // Assumes rb.useGravity = false
        rb.AddForceAtPosition(Physics.gravity / Mathf.Max(1, floaters),
                                transform.position,
        ...
        if (depth > 0f)
        {
            ...
            rb.AddForceAtPosition(buoyancy, transform.position, ForceMode.Acceleration);
            ...
            rb.AddForce(horizDragForce, ForceMode.Acceleration);
            ...
            rb.AddForce(verticalDragForce, ForceMode.Acceleration);

            // --- Angular drag ---
            Vector3 w = rb.angularVelocity;
            Vector3 yaw = Vector3.Project(w, rb.transform.up);
            Vector3 rollPitch = w - yaw;

            Vector3 angularDamping = -(yaw + rollPitch) * waterAngularDrag * disp;
            rb.AddTorque(angularDamping, ForceMode.Acceleration);
        }
    }
}
